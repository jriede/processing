...
class Centre {
  ...
  void display() {
    ...
    beginShape();
    angle = 0;
    x1 = x+r*sin(angle) + random(0,5);
    y1 = y+r*cos(angle) + random(0,5);
    
    coords[0][0] = x;
    coords[0][1] = y;
    
    curveVertex(coords[0][0],coords[0][1]);
    
    coords[1][0] = x1;
    coords[1][1] = y1;
    
    curveVertex(coords[1][0],coords[1][1]);
    
    for (i=2;i<=corners;i++) {
      angle = i * TWO_PI/float(corners);
      coords[i][0] = x+r*sin(angle) + random(0,5);
      coords[i][1] = y+r*cos(angle) + random(0,5);
      
      curveVertex(coords[i][0],coords[i][1]);
      //ellipse(x1,y1,3,3);
    }
    //curveVertex(x+r*sin(angle) + random(0,5),  y+r*cos(angle) + random(0,5));
     curveVertex(coords[1][0],coords[1][1]);
    endShape(CLOSE);
